...
def extend_general_task(task_name, num_per_subcategory, prompts):
    ...
    #     raise NotImplementedError(f"No template defined for task '{task_name}'")
    ...
    for subtask, examples in task_data.items():
        ...
        print(f"[INFO] Extending {task_name}/{subtask} with {num_per_subcategory} new samples...")
...
def extend_counting_task(num_new_subtasks, prompts):
    ...
    # Show GPT a sample of existing examples (first 2 subtasks)
    sample_subtasks = {k: task_data[k] for k in list(task_data.keys()) if k != "prompt"} 
    sample_json = json.dumps(sample_subtasks, indent=4)
    ...
    print(f"[INFO] Requesting {num_new_subtasks} new counting subtasks in batch...")
    ...
    try:
        ...
        print(f"[ERROR] Could not parse GPT output for counting batch: {e}")
        print("Raw GPT output:", message)
...
if __name__ == "__main__":
    ...
    if args.task == "counting":
        prompts = extend_counting_task(args.num, prompts)
    else:
        prompts = extend_general_task(args.task, args.num, prompts)
